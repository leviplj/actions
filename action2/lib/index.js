...
/***/ (function(__unused_webpack_module, exports, __nccwpck_require__) {
  ...
  Object.defineProperty(exports, "__esModule", ({ value: true }));
  exports.updateTenantMetadata = exports.getTenantMetadata = exports.getToken = void 0;
  const axios_1 = __importDefault(__nccwpck_require__(6545));
  const getToken = async (clientId, secret) => {
      const tokenData = {
          clientId,
          secret,
      };
      const res = await axios_1.default.post('https://api.us.frontegg.com/auth/vendor/', tokenData, {
          headers: {
  ...
  const getTenantMetadata = async (tenantId, token) => {
      const res = await axios_1.default.get(`https://api.us.frontegg.com/tenants/resources/tenants/v1/${tenantId}`, {
          headers: {
              Accept: 'application/json',
              Authorization: `Bearer ${token}`,
          },
      });
      return res.data.length ? JSON.parse(res.data[0].metadata) : null;
  };
  exports.getTenantMetadata = getTenantMetadata;
  const updateTenantMetadata = async (tenantId, metadata, token) => {
      const data = {
          metadata,
      };
      const res = await axios_1.default.post(`https://api.us.frontegg.com/tenants/resources/tenants/v1/${tenantId}/metadata`, data, {
          headers: {
              Accept: 'application/json',
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
  ...
  exports.updateTenantMetadata = updateTenantMetadata;
  ...
  const frontegg_1 = __nccwpck_require__(4866);
  // interface Workspace {
  //   workspace: string;
  //   version: number;
  //   updated?: Date;
  ...
  //   return res.data.token;
  // };
  // const getTenantMetadata = async (tenantId: string, token: string) => {
  //   const res = await axios.get(
  //     `https://api.us.frontegg.com/tenants/resources/tenants/v1/${tenantId}`,
  //     {
  //       headers: {
  //         Accept: 'application/json',
  //         Authorization: `Bearer ${token}`,
  ...
  //   return res.data;
  // };
  const addWorkspace = async (workspaces, branchName) => {
      const branchWorkspace = workspaces.find((x) => x.name === branchName);
      if (branchWorkspace) {
          return workspaces;
      }
      return [
  ...
  const updateWorkspace = async (workspaces, branchName) => {
      const branchWorkspace = workspaces.find((x) => x.name === branchName);
      if (!branchWorkspace) {
          throw new Error(`Workspace ${branchName} not found on frontegg.`);
      }
      branchWorkspace.values['updated'] = new Date();
      return workspaces;
  };
  const deleteWorkspace = async (workspaces, branchName) => {
      return workspaces.filter((x) => x.name !== branchName);
  };
  const run = async () => {
      core.debug('Running Action 2');
  };
  run();
